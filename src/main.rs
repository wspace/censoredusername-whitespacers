    ...
    loop {
        match args.next() {
            Some(arg) => match arg.as_ref() {
                ...
                "-h" | "--help" => return Err("Usage: wsc PROGRAM [-h | -i INFILE | -o OUTFILE | [-t | -e STRATEGY | -d DUMPFILE | -c] | -f FORMAT | -p | --ignore-overflow | --unchecked-heap | --no-fallback | --no-implicit-exit]
